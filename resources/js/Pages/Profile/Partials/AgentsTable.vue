                ...
                <div class="bg-white border-gray-200 dark:bg-gray-900">
                    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto mb-3">
                        ...
                        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-2" id="navbar-search">
                            <Link :href="route('agent.create.form')" class="text-white bg-emerald-800 hover:bg-emerald-900 font-medium rounded-lg text-sm px-5 py-2 text-center inline-flex items-center dark:focus:ring-[#3b5998]/55">
...
export default {
    ...
    methods: {
        ...
        searchUsers() {
            const searchTerm = this.searchTerm.toLowerCase();
            this.rowsToShow = this.productList.filter((agent) => {
                const lowerCaseOwnerName = agent.name.toLowerCase();

                // Check if the searchTerm matches the owner_name
                return lowerCaseOwnerName.includes(searchTerm);
            });
        },
        nextPage() {
